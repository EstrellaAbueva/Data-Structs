...
class LinkedList : public List {
    ...
    public:
        ...
        void addHead(int num) {
            ...
            newest->next = head;
            head = newest;
            if (tail == NULL) {//if wala ni tail liek usa ra ang sud sa linked list
                tail = newest;
            }
            index++;
        }
        ...
        // START HERE
        int removeAt(int pos) {// pwede ra ni same sa rmeove ilisan lang ang first na if statement 
        ...
        int removeTail() {
            ...
            node* currnode = head;//assigned head
            node* prevnode = NULL;// end part list

            if(head == NULL){ //meaning list is empty
                return elem;//0
            }else if(head->next == NULL){//there are possibly 1 items
                removeHead();
            }else{
                while(currnode->next != NULL){//will loop until reaches the tail starting from the 2nd element
                    prevnode = currnode;// prevnode = head
                    currnode = currnode->next;   
                }
                index--;// decrement
            }

            elem = currnode->element; //previous value of the tail
            free(currnode); //deallocate the memory or the previous tail
            tail = prevnode;//assignning new taul
            prevnode->next = NULL;
        ...
        bool isEmpty() {
            node* first = head;//assigned head
            node* next = NULL;
            if(first == NULL && next == NULL){ //check if both head and tail are empty 
                return true;
            }else
                return false;
            //index == 0
        ...
        void clear() {
            ...
            while (current != NULL){
                ...
                current = head; // assign a new head
            }
            /*
            while (!isEmpty()){
                removeHead(); //remove head
